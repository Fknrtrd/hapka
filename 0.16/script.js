...
var irrationalityIterator = 0;

function irrationality() {

	var openWord = "irrationalityOpen";
	var spoilerContent = "irrationalityContent";
	var closeWord = "irrationalityClose";

	if (irrationalityIterator === 0) {
		viewSpoiler(openWord, spoilerContent, closeWord);
		irrationalityIterator++;
	} else {
		closeSpoiler(openWord, spoilerContent, closeWord);
		irrationalityIterator--;
	}
	...
	function viewSpoiler(openWord, spoilerContent, closeWord) {
		document.getElementById(spoilerContent).innerHTML = '<div class="spoilerBox"><p>Иррациональность (от лат. irrationalis — «неразумное») — многозначный термин.</p><br><p>Значения в:</p><br><p>&#8226; философии:</p><br><p>Cвойство по значению прилагательного иррациональный; отсутствие закономерности, невозможность постижения чего-либо разумом.</p><br><p>&#8226; математике:</p><br><p>Иррациональность — свойство, невозможность точного числового выражения — см. Иррациональное число, которое может быть представлено в виде десятичной дроби только бесконечной и непериодической .</p><br><p>&#8226; логике:</p><br><p>Иррацинальное — находящееся за пределами разума, противоречащее логике. Обычно противопоставляется рациональному как разумному, целесообразному, обоснованному.</p><br><p>&#8226; психологии:</p><br><p>Иррациональный (по типологии Юнга) — класс функций человеческой психики, лежащих «за пределами разума» — ощущение и интуиция.</p><br>Источник:<br><a target="_blank" href="https://ru.wikipedia.org/wiki/%D0%98%D1%80%D1%80%D0%B0%D1%86%D0%B8%D0%BE%D0%BD%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D1%8C">https://ru.wikipedia.org/wiki/Иррациональность</a><br></div><br>';
		document.getElementById(closeWord).innerHTML = document.getElementById(openWord).innerHTML;
		//document.getElementById(closeWord).setAttribute("class", "spoilerWord");
		document.getElementById(closeWord).setAttribute("class", "spoilerWordActive");
		document.getElementById(openWord).setAttribute("class", "spoilerWordActive");
...
var algorithmIterator = 0;

function algorithm() {

	var openWord = "algorithmOpen";
	var spoilerContent = "algorithmContent";
	var closeWord = "algorithmClose";

	if (algorithmIterator === 0) {
		viewSpoiler(openWord, spoilerContent, closeWord);
		algorithmIterator++;
	} else {
		closeSpoiler(openWord, spoilerContent, closeWord);
	...
	function viewSpoiler(openWord, spoilerContent, closeWord) {
		document.getElementById(spoilerContent).innerHTML = '<div class="spoilerBox"><p>Алгори́тм (лат. al­go­rithmi — от арабского имени математика Аль-Хорезми) — конечная совокупность точно заданных правил решения произвольного класса задач или набор инструкций, описывающих порядок действий исполнителя для решения некоторой задачи. В старой трактовке вместо слова «порядок» использовалось слово «последовательность», но по мере развития параллельности в работе компьютеров слово «последовательность» стали заменять более общим словом «порядок». Независимые инструкции могут выполняться в произвольном порядке, параллельно, если это позволяют используемые исполнители.</p><br><p>Ранее в русском языке писали «алгорифм», сейчас такое написание используется редко, но, тем не менее, имеет место исключение (нормальный алгорифм Маркова).</p><br><p>Часто в качестве исполнителя выступает компьютер, но понятие алгоритма необязательно относится к компьютерным программам, так, например, чётко описанный рецепт приготовления блюда также является алгоритмом, в таком случае исполнителем является человек (а может быть и некоторый механизм, ткацкий станок, и пр.).</p><br><p>Можно выделить алгоритмы вычислительные (о них в основном идет далее речь), и управляющие. Вычислительные по сути преобразуют некоторые начальные данные в выходные, реализуя вычисление некоторой функции. Семантика управляющих алгоритмов существенным образом может отличаться и сводиться к выдаче необходимых управляющих воздействий либо в заданные моменты времени, либо в качестве реакции на внешние события (в этом случае, в отличие от вычислительного алгоритма, управляющий может оставаться корректным при бесконечном выполнении).</p><br><p>Понятие алгоритма относится к первоначальным, основным, базисным понятиям математики. Вычислительные процессы алгоритмического характера (арифметические действия над целыми числами, нахождение наибольшего общего делителя двух чисел и т. д.) известны человечеству с глубокой древности. Однако в явном виде понятие алгоритма сформировалось лишь в начале XX века.</p><br><p>Частичная формализация понятия алгоритма началась с попыток решения проблемы разрешения (нем. Entscheidungsproblem), которую сформулировал Давид Гильберт в 1928 году. Следующие этапы формализации были необходимы для определения эффективных вычислений или «эффективного метода»; среди таких формализаций — рекурсивные функции Геделя — Эрбрана — Клини 1930, 1934 и 1935 гг., λ-исчисление Алонзо Чёрча 1936 г., «Формулировка 1» Эмиля Поста 1936 года и машина Тьюринга.</p><br>Источник:<br><a target="_blank" href="https://ru.wikipedia.org/wiki/%D0%90%D0%BB%D0%B3%D0%BE%D1%80%D0%B8%D1%82%D0%BC">https://ru.wikipedia.org/wiki/Алгоритм</a><br><br><span>Продолжительность 6:34</span><br><a target="_blank" href="https://www.youtube.com/watch?v=6U93giIDI9k">https://www.youtube.com/watch?v=6U93giIDI9k</a><br><div class="resp-wrapper"><iframe class="resp-iframe" src="https://www.youtube.com/embed/6U93giIDI9k" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div></div><br>';
		document.getElementById(closeWord).innerHTML = document.getElementById(openWord).innerHTML;
		//document.getElementById(closeWord).setAttribute("class", "spoilerWord");
		document.getElementById(closeWord).setAttribute("class", "spoilerWordActive");
		document.getElementById(openWord).setAttribute("class", "spoilerWordActive");
